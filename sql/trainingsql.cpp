    ...
    bool intelligence, bool useEszett, bool useUmlaut)
{
    ...
    if (intelligence) {
        ...
        while (query.next()) {
            ...
            if (unit == 0
                || (unit == 1 && counterToNewLine > NUM_TOKEN_UNTIL_NEW_LINE)) {
        ...
        if (errorsExist && (queryCounter % NUM_INTELLIGENT_QUERYS) != 0) {
            ...
            if ((lesson % 100) != LAST_LESSON || type != 0) {
                ...
                sqlString.append("(" + tableName + ".content_lesson % 100) >= "
                    + QString::number(BORDER_LESSON_IS_SENTENCE) + " AND ("
                    + tableName + ".content_lesson % 100) <= "
                    + QString::number(LAST_LESSON));
            }
...
QString TrainingSql::replaceRoutine(
    QString content, bool useEszett, bool useUmlaut, int lesson, int type)
{
    ...
    if (replaceSetting != "") {
        QStringList replaceList
            = replaceSetting.split(",", QString::SkipEmptyParts);
        QStringList searchReplace;

        for (int i = 0; i < replaceList.size(); ++i) {
            searchReplace
                = replaceList.at(i).split("=", QString::SkipEmptyParts);
