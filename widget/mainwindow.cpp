...
// -----------
    ...
    // Menu bar actions
    settingsAction = new QAction(
        QIcon(":/img/menu_settings.png"), tr("&Grundeinstellungen"), this);
#endif
    exitAction = new QAction(tr("&Beenden"), this);
    evalAction = new QAction(
        QIcon(":/img/menu_evaluation.png"), tr("&Lernstatistik"), this);
    ...
    connect(
        gameAction, SIGNAL(triggered()), this, SLOT(toggleStartToAbcrain()));
...
void MainWindow::about()
{
    ...
    if (!query.exec("SELECT * FROM db_version ORDER BY version DESC;")) {
        ...
        errorMessage->showMessage(
            ERR_DB_VERSION_READABLE, TYPE_WARNING, CANCEL_NO);
    ...
    versionText = tr("Programmversion ") + softwareVersion + "<br>"
        + tr("Datenbankversion ") + dbVersion;
    if (APP_PORTABLE) {
        versionText.append("<br>" + tr("Portable Version"));
    }

    QString infoHeadline = tr("Der intelligente 10-Finger-Schreibtrainer");
    QString infoSubHeadline = tr("Im Internet: ") + "<a href=\""
        + QString(APP_URL) + "\">" + QString(APP_URL) + "</a>";
    QString infoText = tr(""
                          "TIPP10 wird unter den Bedingungen der GNU General "
                          "Public License veroeffentlicht und "
                          "ist kostenlos. Sie muessen dafuer nichts bezahlen, "
                          "egal wo Sie die Software herunter"
                          "laden!");
    QString supportHeadline = tr("TIPP10 unterstuetzen");
    QString supportText
        = tr(""
             "Sollte Ihnen TIPP10 gut gefallen, wuerden wir uns sehr freuen, "
             "wenn Sie das "
             "Projekt mit einem Geldbetrag unterstuetzen wuerden. Nur so kann "
             "TIPP10 auch weiterhin kostenlos angeboten, gepflegt und "
             "weiterentwickelt werden. Vielen Dank!")
    ...
    QString companyText = QString(APP_ORGANIZATION) + ", " + "<a href=\""
        + "http://www.thielicke.org" + "\">" + "http://www.thielicke.org"
        + "</a>";
...
void MainWindow::createStart()
{
    ...
    connect(startWidget, SIGNAL(trainingClicked(int, int, QString)), this,
        SLOT(toggleStartToTraining(int, int, QString)));
...
void MainWindow::deleteStart()
{
    ...
    disconnect(startWidget, SIGNAL(trainingClicked(int, int, QString)), this,
        SLOT(toggleStartToTraining(int, int, QString)));
...
void MainWindow::createTraining(int lesson, int type, QString name)
{
    ...
    connect(trainingWidget,
        SIGNAL(lessonReady(int, int, QList<QChar>, QList<int>)), this,
        SLOT(toggleTrainingToEvaluation(int, int, QList<QChar>, QList<int>)));
    connect(trainingWidget, SIGNAL(lessonCanceled()), this,
        SLOT(toggleTrainingToStart()));
}
...
void MainWindow::deleteTraining()
{
    ...
    trainingStarted = false;
    disconnect(trainingWidget,
        SIGNAL(lessonReady(int, int, QList<QChar>, QList<int>)), this,
        SLOT(toggleTrainingToEvaluation(int, int, QList<QChar>, QList<int>)));
    disconnect(trainingWidget, SIGNAL(lessonCanceled()), this,
        SLOT(toggleTrainingToStart()));
    ...
    int row, int type, QList<QChar> charList, QList<int> mistakeList)
{
    ...
    connect(evaluationWidget, SIGNAL(readyClicked()), this,
        SLOT(toggleEvaluationToStart()));
    if (!isMaximized() && height() < APP_HEIGHT_STANDARD) {
        resize(APP_WIDTH_STANDARD, APP_HEIGHT_STANDARD);
    }
    setMinimumSize(APP_WIDTH_STANDARD, APP_HEIGHT_STANDARD);
...
void MainWindow::deleteEvaluation()
{
    ...
    disconnect(evaluationWidget, SIGNAL(readyClicked()), this,
        SLOT(toggleEvaluationToStart()));
...
void MainWindow::createAbcrain()
{
    ...
    connect(abcRainWidget, SIGNAL(readyClicked()), this,
        SLOT(toggleAbcrainToStart()));
}

void MainWindow::deleteAbcrain()
{
    stackedWidget->removeWidget(abcRainWidget);
    disconnect(abcRainWidget, SIGNAL(readyClicked()), this,
        SLOT(toggleAbcrainToStart()));
...
#else
    resize(
        settings.value("size", QSize(APP_WIDTH_STANDARD, APP_HEIGHT_STANDARD))
            .toSize());
