            ...
            *lessonData = lineLessonContent->toPlainText().split(
            ...
            if (!startSql->getOwnLesson(currentLessonId, lineLessonName,
                ...
                errorMessage->showMessage(
                    ERR_USER_LESSON_GET, TYPE_INFO, CANCEL_OPERATION);
                return;
            }
        }
    }
...
void LessonDialog::createControls()
{
    ...
    labelLessonNotices = new QLabel(
        tr("<u>Erlaeuterung</u>"
           ...
           "<b>Satzdiktat</b> - die "
           "einzelnen Zeilen (Saetze) werden wie hier eingegeben mit einem "
           "Zeilenumbruch am "
           "Ende diktiert.<br>&nbsp;<br>"
           "<b>Wortdiktat</b> - die einzelnen Zeilen (Worte) werden mit "
           "Leerzeichen getrennt "
           "diktiert, ein Zeilenumbruch erfolgt automatisch nach mindestens ")
        + QString::number(NUM_TOKEN_UNTIL_NEW_LINE)
        + tr(" diktierten "
             "Schriftzeichen."));
    labelLessonNotices->setWordWrap(true);
    labelLessonNotices->setMaximumWidth(180);
    labelLessonUnit = new QLabel(tr(
        "<b>Was passiert bei aktivierter "
    ...
    labelLessonUnitRadio
        = new QLabel(tr("Das Diktat soll gefuehrt werden als:"));
...
void LessonDialog::clickSave()
{
    ...
    if (lineLessonName->text() == "") {
        QMessageBox::information(
            this, APP_NAME, tr("Bitte geben Sie der Lektion einen Namen\n"));
        return;
    }
    ...
    if (!startSql->updateOwnLesson(currentLessonId, lineLessonName->text(),
        ...
        ErrorMessage* errorMessage = new ErrorMessage(this);
        errorMessage->showMessage(
            ERR_USER_LESSON_ADD, TYPE_INFO, CANCEL_OPERATION);
        return;
    }
