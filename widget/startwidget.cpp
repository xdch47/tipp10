...
void StartWidget::createGroupLesson()
{
    ...
    */
    ...
    lessonNew
        = new QAction(QIcon(":/img/menu_new.png"), tr("&Neue Lektion"), this);
    lessonImport = new QAction(
        QIcon(":/img/menu_import.png"), tr("Lektion &importieren"), this);
    lessonExport = new QAction(
        QIcon(":/img/menu_export.png"), tr("Lektion &exportieren"), this);
    lessonEdit = new QAction(
        QIcon(":/img/menu_edit.png"), tr("Lektion &editieren"), this);
    lessonDel = new QAction(
        QIcon(":/img/menu_del.png"), tr("Lektion &loeschen"), this);
...
void StartWidget::createGroupError()
{
    ...
    checkErrorCorrect->setToolTip(
        tr("Tippfehler muessen zusaetzlich ueber die\n"
    ...
    checkErrorImage->setToolTip(
        tr("Bei jedem Tippfehler wird das "
    ...
    h2.setPointSize(7);
    labelIntelligence = new QLabel(
        tr("*Aktivieren Sie diese Option, "
           "wenn die Texte der Lektion nicht in ihrer vorgesehenen Reihenfolge "
           "diktiert, "
    ...
    checkIntelligence->setToolTip(
        tr("Anhand der aktuellen Fehlerquoten aller Schriftzeichen "
           "werden\ndie Worte und Saetze des Diktats in Echtzeit "
           "ausgewaehlt.\nIst die Intelligenz dagegen "
           "deaktiviert, werden die Texte der Lektion\nstets in der gleichen "
           "Reihenfolge diktiert."));
...
void StartWidget::createGroupSupport()
{
    ...
    checkHelpers->setToolTip(
        tr("Zur visuellen Unterstuetzung "
    ...
    checkKeySelectionStart->setToolTip(tr(
        "Zur visuellen Unterstuetzung "
...
void StartWidget::createConnections()
{
    ...
    connect(
        checkIntelligence, SIGNAL(clicked()), this, SLOT(toggleIntelligence()));
    ...
    connect(
        tabLessons, SIGNAL(currentChanged(int)), this, SLOT(toggleTabs(int)));
...
void StartWidget::fillLessonList(bool themeChanged = false)
{
    ...
    // Open themes list
    // --------------------
    if ((themesNum = lessonSql->fillThemes(
             comboTheme, &arrayThemes, languageLesson, tr("Alle")))
        == -1) {
        // Error message
        ErrorMessage* errorMessage = new ErrorMessage(this);
        errorMessage->showMessage(ERR_LESSONS_EXIST,
...
void StartWidget::clickTraining()
{
    ...
    if (selectedLesson == -1 || listSize == 0) {
        ...
        // -> error message
        ErrorMessage* errorMessage = new ErrorMessage(this);
        errorMessage->showMessage(ERR_LESSONS_SELECTED,
...
void StartWidget::doubleClickLesson(QListWidgetItem* item) { clickTraining(); }
...
void StartWidget::clickImportLesson()
{
    ...
    QString path
        = fd->getOpenFileName(this, tr("Bitte waehlen Sie eine Textdatei aus"),
    ...
    if (!file.open(QIODevice::ReadOnly | QIODevice::Text)) {
        // Error message
        ErrorMessage* errorMessage = new ErrorMessage(this);
        errorMessage->showMessage(ERR_USER_IMPORT_READ,
            ErrorMessage::Type::Info, ErrorMessage::Cancel::Operation);
        return;
    }
    ...
    if (lessonData->size() < 2) {
        // Error message
        ErrorMessage* errorMessage = new ErrorMessage(this);
        errorMessage->showMessage(ERR_USER_IMPORT_EMPTY,
            ErrorMessage::Type::Info, ErrorMessage::Cancel::Operation);
        return;
    }
...
void StartWidget::clickExportLesson()
{
    ...
    QString path = fd->getSaveFileName(this,
        tr("Bitte geben Sie den Speicherort fuer eine Textdatei ein"),
        fileNameEncoded);
    ...
    if (!file.open(QFile::WriteOnly | QIODevice::Text)) {
        // Error message
        ErrorMessage* errorMessage = new ErrorMessage(this);
        errorMessage->showMessage(ERR_USER_EXPORT_WRITE,
            ErrorMessage::Type::Info, ErrorMessage::Cancel::Operation);
        return;
    }
...
void StartWidget::clickDeleteLesson()
{
    switch (QMessageBox::question(this, APP_NAME,
        tr("Wollen Sie die Lektion wirklich loeschen und damit auch "
           "alle aufgezeichneten Daten, die im Zusammenhang mit dieser Lektion "
           "stehen? "
           "\n\n"),
...
// the application and company name see main function)
    ...
    spinLimitTime->setValue(
        settings.value("spin_time", LESSON_TIMELEN_STANDARD).toInt());
    spinLimitToken->setValue(
        settings.value("spin_token", LESSON_TOKENLEN_STANDARD).toInt());
    settings.endGroup();
