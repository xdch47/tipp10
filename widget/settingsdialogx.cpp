    ...
    : QDialog(parent)
{
    ...
    connect(
        buttonLessonsReset, SIGNAL(clicked()), this, SLOT(deleteUserData()));
...
void SettingsDialog::createGroupKeyboardLayout()
{
    ...
    radioLayoutWin->setToolTip(tr("Waehlen Sie diese Option, wenn Sie "
                                  "eine standardmaessige Windows-Tastatur "
...
void SettingsDialog::createGroupUserReset()
{
    ...
    buttonLessonsReset
        = new QPushButton(tr("&Gespeicherte Lektionen zuruecksetzen"));
    buttonLessonsReset->setToolTip(
        tr("Hier koennen Sie saemtliche "
           "gespeicherte Lektionendaten\n(die Lektionen werden so in den "
           "urspruenglichen Zustand,\n"
           "wie nach der Installation, versetzt)"));

    // Button "Reset user data"
    buttonCharsReset
        = new QPushButton(tr("&Gespeicherte Schriftzeichen zuruecksetzen"));
    buttonCharsReset->setToolTip(
        tr("Hier koennen Sie saemtliche "
           "gespeicherte Schriftzeichen (Tippfehler)\n(die Schriftzeichen "
           "werden so in den urspruenglichen Zustand,\n"
           "wie nach der Installation, versetzt)"));
...
void SettingsDialog::createGroupDatabase()
{
    ...
    labelDatabasePath
        = new QLabel(tr("Verzeichnispfad zur Datenbank \"tipp10v2.db\":"));
    lineDatabasePath = new QLineEdit();
    buttonDatabasePath = new QPushButton(tr("..."));
    buttonDatabasePath->setMaximumWidth(30);
    labelDatabaseExplain
        = new QLabel(tr("Wenn im angegebenen Verzeichnis keine Datei "
                        "\"tipp10v2.db\" existiert, "
...
void SettingsDialog::setFontButtonLabel()
{
    buttonSetFont->setText(
        tickerFont.family() + ", " + QString::number(tickerFont.pointSize()));
}
    ...
    case 0:
        ...
        if (!userSql->deleteUserLessonList()) {
            ...
            errorMessage->showMessage(
                ERR_USER_LESSONS_FLUSH, ErrorMessage::Type::Warning, ErrorMessage::Cancel::Operation);
            return;
        }
        if (!userSql->deleteUserChars()) {
            ...
            ErrorMessage* errorMessage = new ErrorMessage(this);
            errorMessage->showMessage(
                ERR_USER_ERRORS_FLUSH, ErrorMessage::Type::Warning, ErrorMessage::Cancel::Operation);
            return;
        }
...
void SettingsDialog::readSettings()
{
    ...
    QSettings settings(
        QSettings::IniFormat, QSettings::UserScope, "portable", "settings");
    ...
    case 0:
        ...
        radioLayoutDvorak->setChecked(
            settings.value("radio_dvorak", false).toBool());
        radioLayoutRistome->setChecked(
            settings.value("radio_ristome", false).toBool());
        break;
    case 1:
    ...
    case 3:
        ...
        radioLayoutNeo->setChecked(settings.value("radio_neo", false).toBool());
        radioLayoutDvorak->setChecked(
            settings.value("radio_dvorak", true).toBool());
        radioLayoutRistome->setChecked(
            settings.value("radio_ristome", false).toBool());
        break;
    case 4:
        ...
        radioLayoutDvorak->setChecked(
            settings.value("radio_dvorak", false).toBool());
        radioLayoutRistome->setChecked(
            settings.value("radio_ristome", true).toBool());
        break;
    default:
        radioLayoutWin->setChecked(settings.value("radio_win", true).toBool());
        radioLayoutMac->setChecked(settings.value("radio_mac", false).toBool());
    ...
    currentDatabasePath
        = settings.value("path", "Fehler/" + dbNameUser).toString();
...
void SettingsDialog::writeSettings()
{
    ...
    settings.beginGroup("database");
    settings.setValue(
        "path", lineDatabasePath->text() + "/" + QString::QString(APP_USER_DB));
    settings.endGroup();
}
