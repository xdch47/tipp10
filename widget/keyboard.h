...
/*!
        ...
        pressed the class receive the written char with a QKeyEvent and emits
   the char with signal keyPressed(). Start and stop the keyboard class with
   function startBoard and stopBoard. When the keyboard is stopped it colors the
   startposition(the eight keys where users fingers musst place on start) and
   the space key. Pause it with function pauseBoard. When the keyboard is paused
   ist colors only the space key. The store the properties of the current key
   and modifier and to access it every time over the paint event, private
   variables are used.
