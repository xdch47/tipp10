...
void TrainingPage::createGroupTickerFont()
{
    ...
    buttonSetFont->setToolTip(tr(
        "Hier koennen Sie die Schriftart der "
    ...
    buttonSetCursorColor->setToolTip(
        tr("Hier koennen Sie die Farbe der Markierung "
    ...
    sliderTickerSpeed->setToolTip(
        tr("Hier koennen Sie die "
           "Geschwindigkeit der Laufschrift veraendern\n(Der Regler ganz links "
           "bedeutet, das "
...
void TrainingPage::createGroupSoundOutput()
{
    ...
    radioSoundcard->setToolTip(
        tr("Waehlen Sie diese Option, wenn Sie "
    ...
    spinMetronomeClock->setToolTip(
        tr("Geben Sie hier an, wie oft pro "
...
void TrainingPage::setFontColor()
{
    QColor color = QColorDialog::getColor(
        buttonSetFontColor->palette().background().color(), this);
    if (color.isValid()) {
...
{
    QColor color = QColorDialog::getColor(
        buttonSetBgColor->palette().background().color(), this);
    if (color.isValid()) {
...
{
    QColor color = QColorDialog::getColor(
        buttonSetCursorColor->palette().background().color(), this);
    if (color.isValid()) {
...
#endif
    radioSoundcard->setChecked(
        settings.value("radio_soundcard", beepDefaultSoundcard).toBool());
    radioSpeaker->setChecked(
        settings.value("radio_speaker", beepDefaultSpeaker).toBool());
...
void TrainingPage::writeSettings()
{
    ...
    settings.setValue("ticker_font_color",
        buttonSetFontColor->palette().background().color().name());
    settings.setValue("ticker_bg_color",
        buttonSetBgColor->palette().background().color().name());
    settings.setValue("ticker_cursor_color",
        buttonSetCursorColor->palette().background().color().name());
    ...
    : QWidget(parent)
{
    ...
    connect(
        buttonDatabasePath, SIGNAL(clicked()), this, SLOT(setDatabasePath()));
...
void DatabasePage::createGroupUserReset()
{
    ...
    buttonLessonsReset = new QPushButton(tr("&Absolvierte Lektionen "
                                            "zuruecksetzen"));
    buttonLessonsReset->setToolTip(tr(
        "Hier koennen Sie saemtliche "
    ...
    buttonCharsReset->setToolTip(
        tr("Hier koennen Sie alle "
...
void DatabasePage::createGroupDatabase()
{
    ...
    labelDatabasePath = new QLabel(
        tr("Verzeichnispfad zur Datenbank") + " \"" + APP_USER_DB + "\":");
...
void DatabasePage::setDatabasePath()
{
    ...
    QString s = QFileDialog::getExistingDirectory(this,
        "Bitte waehlen Sie ein Verzeichnis aus...", lineDatabasePath->text());
...
void DatabasePage::readSettings()
{
    ...
    currentDatabasePath
        = settings.value("pathpro", "Fehler/" + dbNameUser).toString();
    if (currentDatabasePath.size() > dbNameUser.size() + 1) {
        lineDatabasePath->setText(currentDatabasePath.left(
            currentDatabasePath.size() - dbNameUser.size() - 1));
    }
    settings.endGroup();
}
...
void DatabasePage::writeSettings()
{
    ...
    settings.beginGroup("database");
    settings.setValue(
        "pathpro", lineDatabasePath->text() + "/" + QString(APP_USER_DB));
    settings.endGroup();
}
...
void OtherPage::createGroupDialogCheck()
{
    ...
    // Check box "Show start window"
    checkTxtMessage = new QCheckBox(
        tr("Hinweis vor Im- und Export von Lektionen anzeigen"));
    ...
    checkIntelligence = new QCheckBox(
        tr("Auf aktivierte Intelligenz "
    ...
    checkLessonPublish = new QCheckBox(
        tr("Vor dem Anlegen einer "
...
void OtherPage::createGroupAdaptation()
{
    ...
    checkNewVersion = new QCheckBox(
        tr("Einmal pro Woche beim Programmstart auf neue\nVersion pruefen"));
...
void OtherPage::readSettings()
{
    ...
    checkLessonPublish->setChecked(
        settings.value("check_lesson_publish", true).toBool());
    checkNewVersion->setChecked(
        settings.value("check_new_version", true).toBool());
...
void LanguagePage::createGroupLanguage()
{
    ...
    labelLessonNotice->setText(
        tr("Die gewaehlten Uebungslektionen sind nicht auf "
