    ...
    : QWidget(parent)
{
    ...
    if (!background.load(":/img/tickerbg.png")) {
        ...
        errorMessage->showMessage(ERR_TICKER_PIC, ErrorMessage::Type::Warning,
...
void TickerBoard::splitLesson()
{
    ...
    txtLessonSplited = txtCompleteLesson.split(
        QChar(TOKEN_NEW_LINE), QString::SkipEmptyParts, Qt::CaseSensitive);
...
void TickerBoard::readSettings()
{
    ...
    colorFont
        = QColor(settings.value("ticker_font_color", QString(TICKER_COLOR_FONT))
                     .toString());
    colorBg = QColor(
        settings.value("ticker_bg_color", QString(TICKER_COLOR_BG)).toString());
    colorCursor = QColor(
