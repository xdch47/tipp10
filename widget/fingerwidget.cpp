...
void FingerWidget::paintEvent(QPaintEvent* event)
{
    ...
    for (int i = 0; i < 9; i++) {
        ...
        fingerX1[i]
            = 20.0 + (currentPos * (((double)this->width() - 40.0) / 10.0));
        fingerX2[i] = 20.0
            + (currentPos * (((double)this->width() - 40.0) / 10.0))
            + (((double)this->width() - 40.0) / 10.0) + additionalWidth - 4.0;
...
void FingerWidget::drawHeadline()
{
    ...
    painter.drawText(QRectF(20.0, 30.0, (double)this->width() - 40.0, 20.0),
        Qt::AlignCenter | Qt::AlignBottom, tr("Fehlerquoten der Finger"));

    painter.setFont(QFont(FONT_STANDARD, FONT_SIZE_PROGRESS));

    painter.drawText(QRectF(20.0, (double)this->height() - 34.0,
                         (double)this->width() - 40.0, 16.0),
        Qt::AlignCenter | Qt::AlignBottom,
    ...
    if (fingerSelected != -1) {
        ...
        drawTooltip(&painter,
            ...
            fingerNames[fingerSelected] + "\n" + tr("Fehlerquote:") + " "
                + QString::number(fingerRates[fingerSelected]) + tr(" %") + "\n"
                + tr("Vorkommen:") + " "
